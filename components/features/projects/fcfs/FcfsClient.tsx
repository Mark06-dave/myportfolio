...
const MotionCard = ({
  ...
  <motion.section
    ...
    className="space-y-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm"
  >
    {children}
  </motion.section>
);

export function FcfsClient() {
  ...
  const averageWaiting =
    schedule.reduce((acc, item) => acc + item.waiting, 0) / (schedule.length || 1);
  const averageTurnaround =
    schedule.reduce((acc, item) => acc + item.turnaround, 0) / (schedule.length || 1);
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)]">
      <div className="mx-auto flex max-w-5xl flex-col gap-10 px-6 py-10 sm:px-10 lg:px-12">
        <motion.header
          className="flex flex-col gap-4 rounded-3xl border border-[var(--border)] bg-[var(--card)] p-6 shadow-sm"
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <h1 className="text-3xl font-semibold tracking-tight">First Come First Serve Visualizer</h1>
            <p className="max-w-3xl text-sm text-[var(--muted-foreground)]">
              Input any set of processes, watch FCFS scheduling compute start times, waiting
              periods, turnaround values, and a responsive Gantt chart.
            </p>
          </div>
        </motion.header>
          ...
          <MotionCard delay={0.05}>
            <header className="flex items-center justify-between">
              <div>
                ...
                <h2 className="text-xl font-semibold tracking-tight">Define your queue</h2>
              </div>
              <button
                type="button"
                onClick={handleReset}
            ...
            >
              <div className="grid gap-2 text-sm">
                <label className="text-[var(--muted-foreground)]">Process name</label>
                <input
                  required
                  value={form.name}
                  onChange={(event) => setForm((prev) => ({ ...prev, name: event.target.value }))}
                  placeholder="e.g. P4"
                  className="rounded-xl border border-[var(--border)] bg-transparent px-4 py-2 text-[var(--foreground)] outline-none focus:border-[var(--primary)]"
                />
              </div>
              <div className="grid gap-2 text-sm sm:grid-cols-2">
                <div className="grid gap-2">
                  <label className="text-[var(--muted-foreground)]">Arrival time</label>
                  <input
                    required
                    min={0}
                    type="number"
                ...
                <div className="grid gap-2">
                  <label className="text-[var(--muted-foreground)]">Burst time</label>
                  <input
                    required
                    min={1}
                    type="number"
              ...
              <button
                type="submit"
                className="mt-2 inline-flex items-center justify-center rounded-2xl bg-[var(--primary)] px-4 py-3 text-sm font-medium text-[var(--primary-foreground)] transition hover:opacity-90"
              >
                Add process
              </button>
            </form>
            ...
            <div className="space-y-3">
              ...
              <div className="flex flex-col gap-2">
                {processes.map((proc) => (
                  <div
                    key={proc.id}
                    className="flex items-center justify-between rounded-2xl border border-dashed border-[var(--border)] px-4 py-3 text-sm"
                  >
          ...
          <div className="space-y-6">
            <MotionCard delay={0.1}>
              <header>
                ...
                <h2 className="text-xl font-semibold tracking-tight">Timeline & metrics</h2>
              </header>
              ...
              <div className="grid gap-4 sm:grid-cols-3">
                ...
                <Metric label="Avg waiting" value={`${averageWaiting.toFixed(1)} units`} />
                <Metric label="Avg turnaround" value={`${averageTurnaround.toFixed(1)} units`} />
              </div>
            </MotionCard>
        ...
        <footer className="pb-8 text-center text-xs text-[var(--muted-foreground)]">
          © {new Date().getFullYear()} Kyle Ando · FCFS experiments.
        </footer>
      </div>
    </div>
  );
